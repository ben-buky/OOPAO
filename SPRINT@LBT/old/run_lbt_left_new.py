...
m = MisRegistration()
m.rotationAngle = 209.278
m.shiftX = 0.16
m.shiftY = 0.028
m.radialScaling = 0.022
m.tangentialScaling = 0.033
...
ind = [5,6,7,8]
ind = np.arange(5,300,4)

LBT.init_SPRINT(mode=ind,                      # 30 is the default mode but can be changed if desired or be multiple modes
                n_mis_reg=5,                  # the number of mis-registration variables. The default is 3, which are shiftX, shiftY, and rotation
...
#%%
from OOPAO.tools.interpolateGeometricalTransformation import (anamorphosisImageMatrix,
                                                          rotateImageMatrix,
...
LBT.dm_lbt.coefs = LBT.M2C_CL[:,0]
LBT_.dm_lbt.coefs = LBT.M2C_CL[:,0]

OPD_in = LBT.dm_lbt.OPD
OPD_exp = LBT_.dm_lbt.OPD

misReg = MisRegistration()
misReg.rotationAngle = 90
OPD_rot = apply_mis_reg(LBT.tel, OPD_in, misReg)


interactive_show(OPD_in*LBT.tel.pupil, OPD_rot*LBT.tel.pupil)